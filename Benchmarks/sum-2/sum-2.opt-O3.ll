...
define dso_local i32 @sum_2() local_unnamed_addr #7 {
...
.lr.ph:                                           ; preds = %0
  ...
  br i1 %.not, label %22, label %.lr.ph.split

.lr.ph.split:                                     ; preds = %.lr.ph
  %7 = icmp eq ptr %4, null
  br i1 %7, label %23, label %.lr.ph.split.split

.lr.ph.split.split:                               ; preds = %.lr.ph.split
  br i1 %6, label %24, label %.lr.ph.split.split.split.preheader

.lr.ph.split.split.split.preheader:               ; preds = %.lr.ph.split.split
  %wide.trip.count = zext nneg i32 %1 to i64
  %min.iters.check = icmp ult i32 %1, 4
  br i1 %min.iters.check, label %.lr.ph.split.split.split.preheader45, label %vector.ph

vector.ph:                                        ; preds = %.lr.ph.split.split.split.preheader
...
vector.body:                                      ; preds = %vector.body, %vector.ph
  ...
  %wide.load = load <4 x i32>, ptr %8, align 4, !tbaa !9
  %9 = getelementptr inbounds i32, ptr %4, i64 %index
  %wide.load44 = load <4 x i32>, ptr %9, align 4, !tbaa !9
  %10 = add nsw <4 x i32> %wide.load44, %wide.load
  %11 = getelementptr inbounds i32, ptr %5, i64 %index
  store <4 x i32> %10, ptr %11, align 4, !tbaa !9
...
middle.block:                                     ; preds = %vector.body
  ...
  br i1 %cmp.n, label %._crit_edge, label %.lr.ph.split.split.split.preheader45

.lr.ph.split.split.split.preheader45:             ; preds = %middle.block, %.lr.ph.split.split.split.preheader
  %indvars.iv.ph = phi i64 [ 0, %.lr.ph.split.split.split.preheader ], [ %n.vec, %middle.block ]
  %.03241.ph = phi i32 [ 0, %.lr.ph.split.split.split.preheader ], [ %14, %middle.block ]
  br label %.lr.ph.split.split.split

.lr.ph.split.split.split:                         ; preds = %.lr.ph.split.split.split.preheader45, %.lr.ph.split.split.split
  %indvars.iv = phi i64 [ %indvars.iv.next, %.lr.ph.split.split.split ], [ %indvars.iv.ph, %.lr.ph.split.split.split.preheader45 ]
  %.03241 = phi i32 [ %21, %.lr.ph.split.split.split ], [ %.03241.ph, %.lr.ph.split.split.split.preheader45 ]
...
._crit_edge:                                      ; preds = %.lr.ph.split.split.split, %middle.block, %0
  %.032.lcssa = phi i32 [ 0, %0 ], [ %14, %middle.block ], [ %21, %.lr.ph.split.split.split ]
  ret i32 %.032.lcssa
}
...
attributes #7 = { nounwind uwtable "min-legal-vector-width"="0" "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="apple-m3" "tune-cpu"="generic" }
attributes #8 = { noreturn nounwind "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="apple-m3" "tune-cpu"="generic" }
